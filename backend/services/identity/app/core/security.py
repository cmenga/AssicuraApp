...
def create_service_token():
    ...
    return jwt.encode(
        claims=claims,
        key=settings.security.service_secret_key,
        algorithm=settings.security.algorithm,
    )
...
def decode_jwt(token: str = Depends(get_token)):
    ...
    if sub not in settings.trusted_services and _type != "service":
        raise HTTPForbidden("Not authorized")
    try:
        payload = jwt.decode(
            token,
            key=settings.trusted_services[sub],
