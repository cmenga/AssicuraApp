...
from api.public.utils import get_current_user
from api.public.utils import get_addresses
from api.public.utils import get_user_session_token

from api.public.schema import UserDataOut
from api.public.schema import AddressDataOut
from api.public.schema import ContactDataIn
from api.public.schema import AddressDataIn
from api.public.schema import ChangePasswordIn
...
async def get_logged_user(auth: AuthenticatedUser, db: DbSession) -> UserDataOut:
    ...
    Returns:
      The function `get_logged_user` is returning a validated `UserDataOut` model for the user retrieved
...
async def get_user_address(
    ...
    of validated AddressDataOut objects.
    ...
    Returns:
      The function `get_user_address` returns a list of `AddressDataOut` objects after fetching and
...
async def update_contact(
    ...
    updating records, committing transactions, and rolling back changes in case of errors.
      item (Annotated[ContactDataIn, Body()]): The `item` parameter in the `update_contact` function is
    of type `Annotated[ContactDataIn, Body()]`. This means that it is expected to receive data in the
    request body that conforms to the `ContactDataIn` model. The `Body()` annotation indicates that the
    data should
    """
...
async def update_address(
    ...
    object that allows you to perform operations
      address (Annotated[AddressDataIn, Body()]): The `address` parameter in the `update_address`
    function is of type `AddressDataIn`. It is used to update the address information for a user. The
...
async def change_password(
    ...
    in the
      item (Annotated[ChangePasswordIn, Body()]): The `item` parameter in the `change_password` function
    represents the data needed to change a user's password. It is of type `Annotated[ChangePasswordIn,
    Body()]`, which likely means it is a request body parameter containing information required to
    change the password. The `ChangePasswordIn

    Returns:
      The function `change_password` returns a JSON response with a success message if the password
    change operation is successful. The response includes a key-value pair with "success" set to True
    and a message indicating that the password was updated successfully.
    """
    fetched_user = get_current_user(db, auth["sub"])
