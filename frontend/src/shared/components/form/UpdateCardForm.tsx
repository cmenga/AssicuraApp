...
function UpdateCardFormErrors({
  ...
  if (errors && errors[field]) return <ErrorMessage message={errors[field]} />;
  return null;
}

function UpdateCardFormSuccess({ message }: { message?: string }) {
  if (message) return <SuccessMessage message={message} />;
  return null;
}
...
export const UpdateCardForm = forwardRef<
  UpdateCardFormHandle,
  UpdateCardFormProps
>(({ isPending, onSubmit, cleanErrors, children }, ref) => {
