...
export default function ChangePassword() {
  ...
  const { errors, message, isPending, submitAction, cleanErrors } =
    useFormStateAction(submitPasswordAction, {
      onSuccess: async () => {
        cleanErrors();
        handleEdit();
        navigate({ to: "/profile" });
  ...
  return (
    ...
    >
      ...
      <UpdateCardForm.Editable>
        ...
        <FormInputPassword
          labelName="Conferma nuova password"
          name="confirm_password"
          onKeyDown={handlePasswordKeyPress}
        >
          {errors?.confirm_password && (
            <ErrorMessage message={errors.confim_password} />
          )}
        </FormInputPassword>
      </UpdateCardForm.Editable>
    </UpdateCardForm>
...
async function submitPasswordAction(
  ...
  switch (response.status) {
    case 403:
      return {
        errors: { body: "Non è stato possibile cambiare passowrd" },
        success: false,
      };
    case 422:
      let errors = {}
