...
export default function Address({ address }: AddressProps) {
  ...
  const { isPending, errors, message, submitAction, cleanErrors } =
    useFormStateAction(submitAddressACtion, {
      onSuccess: async () => {
        await updateAddress();
        cleanErrors();
        handleEdit();
...
export async function submitAddressACtion(
  ...
  switch (response.status) {
    case 401:
      return {
        errors: {
          body: "La sessione potrebbe essere scaduta, la preghiamo di rieffettuare il login",
        },
        success: false,
      };
    case 404:
  ...
  return {
    message: "La residenza è stata cambiata con successo",
    success: true,
  };
}
...
async function updateAddress() {
  ...
  if (response.status === 200) {
    store.dispatch<AddressModel>("address", (prev) => ({
      ...prev,
      ...response.data,
    }));
    return;
