...
import type { ActionResponse, DriverLicenseModel, DropdownOptions } from "@/shared/type";
...
export default function FromDriverLicense({ dateOfBirth, onClose }: FromDriverLicenseProps) {
    const { errors, cleanErrors, isPending, submitAction } = useFormStateAction(submitLicenseAction, {
        onSuccess: async () => { cleanErrors(); await updateDriverLicense(); onClose(); }
    });

    return (<form onSubmit={submitAction} className="max-w-4xl mx-auto p-6">
        <div className="space-y-6">
            <FormHeader
                title="Patente"
                description="Dettagli della patente"
            />
            ...
            <input className="hidden" value={dateOfBirth} onChange={(e) => { return e.preventDefault(); }} name="date_of_birth" />
...
async function submitLicenseAction(formData: FormData): Promise<ActionResponse> {
...
async function updateDriverLicense() {
    ...
    if (response.status === 200) {
        const newData: DriverLicenseModel[] = response.data.map((element: DriverLicenseModel) => ({ ...element }));
